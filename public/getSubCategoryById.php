<?php
	...
	$result = $conn->query("SELECT category_id,'' sub_category_name,content.id content_id, content.name content_name, Concat('http://betweenlifestyle.com/senshare/upload/optimize/',SUBSTRING_INDEX(`content`.image_path, '/', -1)) opt_img FROM content WHERE category_id = '".$_POST["id"]."' ORDER BY id DESC  limit ".$_POST['start'].",".$_POST['end']);
//$result = $conn->query("SELECT distinct `main_category_data`.id category_id,`content`.id content_id,`content`.name content_name,`content`.image_path,Concat('http://203.223.131.51:81/between_v1/upload/optimize/',SUBSTRING_INDEX(`content`.image_path, '/', -1)) opt_img FROM `content` on `content`.Category_Id = `main_category_data`.id where `main_category_data`.id ='".$_POST["id"]."' LIMIT 0 , 50;");
		...
		foreach($conn->query("SELECT count(id) as total FROM `content` where category_id='".$_POST["id"]."'") as $rowtoal)
{
$total = $rowtoal["total"];
}
