<?php
	...
	$result = $conn->query("SELECT category_id,'' sub_category_name,content.id content_id, content.name content_name, Concat('http://betweenlifestyle.com/senshare/upload/optimize/',SUBSTRING_INDEX(`content`.image_path, '/', -1)) opt_img FROM content WHERE category_id = '".$_POST["id"]."' and type = '".$_POST["type"]."' ORDER BY id DESC  limit ".$_POST['start'].",".$_POST['end']);
